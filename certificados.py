...
def generar_pdf(data, nombre_archivo):
    ...
    tiempo_voluntariado = calcular_tiempo(
      ...
      texto = (
        ...
        f"Mediante el presente, Crea+ deja constancia que <b>{data['NOMBRE_SUNAT']}</b> con DNI <b>{data['DNI']}</b>, participó como voluntaria/o en el área/equipo <b>{data['¿En qué área o equipo participaste?']} desde el <b>{fecha_vinculacion}</b> al <b>{fecha_desvinculacion}</b>  en el rol de <b>{data['¿Qué rol desarrollaste dentro de la organización?']}</b>, cumpliendo con {tiempo_voluntariado}.<br/><br/>"
